            ...
            if link != ():
                ...
                if "http" not in link and "www." not in link:
                    ...
                    if not self.is_playlist_valid([link]):
                        await self.bot.say("That is not a yotube link... Dum dragon your are XD")
                        return
                if await self.is_alone_or_admin(msg):
                    ...
                    await self.bot.say("Okay!, I will play this for you :D Rawr~")
                else:
                    self.playlist = []
                    self.current = -1
                    if not self.queue: await self.bot.say("I added this song to the queue for you Rawr~")
                    self.queue.append(link)
            else:
