...
class MentionTracker:
    ...
    async def mention(self,ctx):
        """Saves your mentions when you're dead... DIBS YOUR COMPUTER"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)

    @mention.command(pass_context=True,name="register")
    async def _mention_register(self,ctx):
        """Register yourself for PM Spam (mention tracking)"""
    ...
    async def tracker(self,message):
        ...
        for mention in mentions:
            if mention != message.author and mention.id in self.mail and mention.status != Status.online:
